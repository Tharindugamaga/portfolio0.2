...
const about = {
  title: "About Me",
  description: "I am Tharindu Gamage, a passionate web developer with experience in building dynamic web applications. I love working with both frontend and backend technologies and continuously learning new tools and frameworks to enhance my skills.",
  info: [
...
const education = [
  {
    title: "My Education",
    description: "My academic background includes Information Technology and Software Engineering studies along with certifications and self-learning projects.",
    items: [
...
const skills = [
  {
    title: "My Skills",
    description: "I have a diverse skill set spanning frontend, backend, databases, and tools for development and design.",
    categories: [
      {
        category: "Frontend",
        items: [
      ...
      },
      {
        ...
        items: [
          ...
          { name: "Java", icon: <span className="font-bold text-red-400">☕</span> },
          { name: "Python", icon: <span className="font-bold text-yellow-300">Py</span> },
        ],
      },
      {
        category: "Databases",
    ...
    >
      <div className="container mx-auto flex flex-col xl:flex-row gap-12">
        ...
        <Tabs defaultValue="experience" className="flex flex-col xl:flex-row gap-6 w-full">
          <TabsList className="flex xl:flex-col gap-4 bg-transparent">
            <TabsTrigger value="experience" className="w-[200px] text-left data-[state=active]:bg-emerald-500 data-[state=active]:text-black rounded-lg px-4 py-2">
              Experience
            </TabsTrigger>
            <TabsTrigger value="education" className="w-[200px] text-left data-[state=active]:bg-emerald-500 data-[state=active]:text-black rounded-lg px-4 py-2">
              Education
            </TabsTrigger>
            <TabsTrigger value="skills" className="w-[200px] text-left data-[state=active]:bg-emerald-500 data-[state=active]:text-black rounded-lg px-4 py-2">
              Skills
            </TabsTrigger>
            <TabsTrigger value="about" className="w-[200px] text-left data-[state=active]:bg-emerald-500 data-[state=active]:text-black rounded-lg px-4 py-2">
              About Me
            </TabsTrigger>
          </TabsList>
          ...
          <div className="flex-1">
            ...
            <TabsContent value="experience">
              <h2 className="text-3xl font-bold mb-2">{experience[0].title}</h2>
              <p className="text-gray-400 mb-6">{experience[0].description}</p>
              <div className="grid md:grid-cols-2 gap-6">
                {experience[0].items.map((job, idx) => (
                  <div key={idx} className="p-6 border rounded-lg bg-black/20">
                    <span className="text-emerald-400 text-sm">{job.duration}</span>
            ...
            <TabsContent value="education">
              <h2 className="text-3xl font-bold mb-2">{education[0].title}</h2>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {education[0].items.map((edu, idx) => (
                  <div key={idx} className="p-6 border rounded-lg bg-black/20">
                    ...
                    <p className="text-gray-400">{edu.position}</p>
                  </div>
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="skills">
              <h2 className="text-3xl font-bold mb-2">{skills[0].title}</h2>
                  ...
                  <div key={idx} className="p-6 border rounded-lg bg-black/20">
                    ...
                    <ul className="space-y-2">
                      {cat.items.map((skill, i) => (
                        <li key={i} className="flex items-center gap-2">{skill.icon} <span>{skill.name}</span></li>
                      ))}
                    </ul>
                  </div>
                ))}
            ...
            <TabsContent value="about">
              ...
              <ul className="space-y-3">
                {about.info.map((item, idx) => (
                  <li key={idx} className="flex justify-between p-3 border rounded-lg bg-black/20">
